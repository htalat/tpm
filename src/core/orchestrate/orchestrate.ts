...
export function shouldAutoRevert(input: AutoRevertInput): boolean {
  ...
  // open-PR strand net (shouldAutoReview) flips it to review instead.
...
// review so the human (or the poller, on the next real signal) owns it.
//
// True only when:
...
export async function runOrchestrate(opts: OrchestrateOpts = {}): Promise<OrchestrateResult> {
  ...
  for (const e of lock.releaseStaleTaskLocks(root, ttl)) {
    if (e.reverted) {
      ...
      logLine("WARN", `${e.qualifiedSlug}: stale lock (was ${e.data.agentId}); in-progress with open PR -> review`);
    }
  }
...
async function runWorkerIteration(opts: WorkerIterationOpts): Promise<OrchestrateResult> {
  ...
  try {
    result = await runWithTimeout(
      ...
      () => {
        ...
        // bound cut it off, the work is delivered — hand it to review
        // instead of reverting to ready (which would re-run and risk a dup PR).
        // A feedback round (before=rework) is left to the poller, same
        // as shouldAutoReview. Otherwise revert to ready for a clean re-claim.
        const snap = snapshotTask(match.task);
        try {
          if (snap.status === "in-progress" && snap.prs > 0 && beforeStatus !== "rework") {
  ...
  //   1. Open-PR strand → review. A task left in-progress with a linked
  ...
  if (matchAfter && shouldAutoReview({ before, after })) {
    log(
      "WARN",
      `${slug}: task left in-progress with a linked PR; flipping to review`,
    );
    try {
