...
test("releaseStaleTaskLocks: stale lock at in-progress WITH a linked PR -> review (not ready)", () => {
  ...
  // risk a duplicate PR — hand it to review instead.
  const root = setupRoot();
  try {
    const taskPath = seedTask(root, "alpha", "001-foo", "in-progress", [
    ...
    assert.equal(statusOf(taskPath), "review");
  } finally {
    rmTempDir(root);
  }
});
