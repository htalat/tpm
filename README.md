...
**React app.** With a build present, `tpm serve` makes the SPA the default UI: a bare `/` 302s to **/app** (React + TypeScript + Tailwind, source in `web/` — its own package; the CLI stays dependency-free). It covers index (harness panel, inbox with actions, agent queue, in flight, projects, activity feed), task detail (rendered sections with inline editors, status-gated actions, PR badge cards, settings), project view (editable Goal/Context/Notes, grouped tasks, new-task form with create-&-ready), per-task runs with a 2s live transcript tail, search, logs, and config. The server-rendered pages stay reachable at their original paths — `/?classic=1` for the index, and each SPA page links its classic counterpart. Data rides the JSON API: reads at `/api/projects[/<slug>]`, `/api/tasks/<path>`, `/api/tasks/<path>/runs`, `/api/inbox`, `/api/queue`, `/api/search?q=`, `/api/vocab`, `/api/events/recent`, `/api/logs`, `/api/config`; mutations POST JSON to `/api/tasks/<path>/<action>`, `/api/projects/<slug>/new-task`, `/api/projects/<slug>/edit`, `/api/bulk/<action>`, `/api/harness/workers` — same action allowlist, argv mapping, and in-process command layer as the HTML forms. Build with `npm --prefix web install && npm --prefix web run build`; without a build, `/` keeps serving the classic UI and `/app` tells you how to build. For UI work, `npm --prefix web run dev` runs Vite on :5173 proxying `/api` + `/events` + `/t` to a running `tpm serve`.
