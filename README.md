...
**React app (preview).** `/app` serves a React + TypeScript + Tailwind SPA (source in `web/`, its own package — the CLI itself stays dependency-free), building page-by-page toward replacing the server-rendered views. It talks to the JSON API: reads at `/api/projects`, `/api/projects/<slug>`, `/api/tasks/<path>`, `/api/inbox`, `/api/queue`, `/api/search?q=`, `/api/vocab`, `/api/events/recent`, `/api/logs`; mutations POST JSON to `/api/tasks/<path>/<action>`, `/api/projects/<slug>/new-task`, `/api/projects/<slug>/edit`, `/api/bulk/<action>`, `/api/harness/workers` — same action allowlist, argv mapping, and in-process command layer as the HTML forms. Build once with `npm --prefix web install && npm --prefix web run build`; `tpm serve` then ships the static bundle at `/app` (without a build, `/app` tells you how to make one). For UI work, `npm --prefix web run dev` runs Vite on :5173 proxying `/api` + `/events` to a running `tpm serve`.
