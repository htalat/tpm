...
- **Fresh main** — after the repo-presence check (and only for execution-prompt runs, not feedback rounds re-entering an existing PR branch), the orchestrator `git fetch origin main`s and inspects the local `main` against `origin/main`: dirty working tree or local main ahead of origin → flip to `blocked` (the reason names the dirty files or the ahead count); local main behind origin → `git checkout main && git pull --ff-only` automatically, then spawn the agent on the fast-forwarded `main` and tell it so in the prompt. This is the orchestrator-side enforcement of the rule in `AGENTS.md` step 4 — PR #120 hit the canonical failure (agent branched off stale local main → conflict at merge), so the spawn refuses to dispatch from a state that could repeat it.
