...
Build the SPA once with `npm --prefix web install && npm --prefix web run build`; `tpm serve` ships the static bundle (without a build, `/app` tells you how to make one). For UI work, `npm --prefix web run dev` runs Vite on :5173 proxying `/api` + `/events` + `/t` to a running `tpm serve`.
