...
- **Notifications** — desktop pings at start/finish/fail, gated by a `notifications` cascade (task > project > global config > default `{ start: false, finish: true, fail: true }`). Channels per platform: macOS uses `osascript`; Windows uses `powershell.exe` and prefers the [BurntToast](https://www.powershellgallery.com/packages/BurntToast) module (`Install-Module BurntToast` — optional but recommended), falling back to the built-in `Windows.UI.Notifications` WinRT API if BurntToast isn't installed; Linux uses `notify-send` (libnotify — on GNOME/KDE and most freedesktop.org desktops; install your distro's `libnotify-bin`/`libnotify` if absent); other platforms log to stderr and skip. `tpm notify <event> <task>` is the same hook as a CLI verb. **Clicking a notification opens the task's `tpm serve` page** (`<serve.url>/t/<project>/<slug>`): on macOS that needs [`terminal-notifier`](https://github.com/julienXX/terminal-notifier) on PATH (`brew install terminal-notifier` — optional; `osascript` has no click-action API, so without it the notification still fires, just non-clickable); on Windows the toast launches the URL as a protocol; on Linux the notification is display-only (`notify-send` has no portable click action). The link only resolves while `tpm serve` is running — see `serve.url` under [Config fields](#config-fields).
...
`tpm serve` starts a localhost web app at `http://127.0.0.1:7777` — a React + TypeScript + Tailwind SPA (source in `web/`, its own package; the CLI itself stays dependency-free) over a JSON API. A bare `/` lands on `/app`.
...
Pages: **index** (Your inbox with promote/reopen buttons, Agent queue, In flight, collapsible per-project lists, the Activity feed from the status journal, and — under `tpm up` — the harness panel: state chip, worker stepper, pause/resume), **task detail** (rendered Context / Plan / Log / Outcome with inline editors, a status-gated Actions rail, PR cards with state / CI / review / mergeable badges, report rendering + LGTM / request-changes for investigations, settings, a `claude --resume` snippet), **project view** (editable Goal / Context / Notes, tasks grouped by status, archived toggle, new-task form with create-&-ready), **runs** (per-task run list + the latest transcript; an in-progress run streams live via a 2-second byte-offset tail), **search** (`/app/search?q=` — slug, title, status, tags, PR URLs, body; snippets highlighted; `archived=1` widens), **logs**, and **config**. Live updates ride `/events` — a server-sent-events stream tailing the status journal (`<root>/.tpm/events.ndjson`) — so a flip made by any process (an agent's `tpm pr`, a cron poller, another tab) lands on open pages within ~1s; tab focus is the catch-all resync.

Data rides the JSON API: reads at `/api/projects[/<slug>]`, `/api/tasks/<path>`, `/api/tasks/<path>/runs`, `/api/inbox`, `/api/queue`, `/api/search?q=`, `/api/vocab`, `/api/events/recent`, `/api/logs`, `/api/config`, `/api/harness`; mutations POST JSON to `/api/tasks/<path>/<action>`, `/api/projects/<slug>/new-task`, `/api/projects/<slug>/edit`, `/api/bulk/<action>`, `/api/harness/workers` — one action allowlist and argv mapping, executed by the in-process command layer, validated by the transition table. Old server-rendered URLs (`/t/…`, `/p/…`, `/search`, `/logs`, `/config`, numeric permalinks like `/t/<project>/12`) permanently redirect to their SPA equivalents, so pre-SPA bookmarks and notification deep links keep working.

**Health checks.** `tpm doctor` is a read-only pass over the slow-to-surface failure modes: config parse, tree readability (project/task counts), pre-rename statuses (`tpm migrate` hint), journal size, stale locks, SPA build freshness (src newer than dist), and the daemon (running? same wire version as this checkout? serving *this* tree with single-writer forwarding?). Warns are advice; only hard failures exit non-zero.

**Single writer.** While a serve/`tpm up` daemon is running, every CLI mutation (`tpm ready`, `tpm pr`, `tpm done`, …) is forwarded into it over `POST /api/cli` and executes in-process — agents, the poller, the web UI, and your shell all write through one process. The journal still records the *caller's* identity (`TPM_AGENT_ID`). With no daemon listening (or a daemon serving a different tree — checked by root), the CLI executes locally exactly as before; `TPM_NO_DAEMON=1` forces local.

Build the SPA once with `npm --prefix web install && npm --prefix web run build`; `tpm serve` ships the static bundle (without a build, `/app` tells you how to make one) — or skip the toolchain entirely and untar the `web-dist` artifact attached to each GitHub release into `web/` (releases build it fresh from the tagged source). For UI work, `npm --prefix web run dev` runs Vite on :5173 proxying `/api` + `/events` + `/t` to a running `tpm serve`.
...
tpm notify <start|finish|fail> <task>     # best-effort desktop notification (macOS osascript / Windows powershell + BurntToast / Linux notify-send; cascade: task > project > global)
